...
import { sanitizeSvg } from "./diagramSvg";
 ...
 * non fiable : mode `strict`, libellés sans HTML, SVG nettoyé puis inséré sans `innerHTML`.
 * Une source invalide affiche un message lisible et la source brute, jamais une page cassée.
 */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const { svg } = await mermaid.render(`diagram-${reactId}`, diagram.mermaid);
        if (cancelled) return;
